...
let cylinderArea = (r, h) => (2*Math.round(Math.PI)*r*h);
console.log(cylinderArea(5, 4));
...
let textCreation = (text) => document.write(`<p>${text}</p>`);
textCreation('some txt');
...
let createUl2 = (text, n) => {
    for (let i = 0; i < n; i++) {
        document.write(`<li>${text}</li>`);
    }
}
createUl2('some txt', 4);
...
let outputsArrayElements = (array) => {
...
console.log(minNumber([19, -8, 23, 0]));
